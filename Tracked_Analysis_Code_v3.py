...
def detect_actuator_activity_segments(
    df,
    window_size: int = 5,
):
    """
    Detect continuous low- and high-activity segments in actuator movement,
    then pick only the largest high-activity segment and the largest low-activity
    segment before it (and after it, if any).  Displays an interactive plot
    so you can fine-tune the start/end of the high segment.

    Returns
    -------
    segments : List[Dict]
        Exactly three dicts (low_before, high, low_after (optional)) with keys:
        - 'type' : 'low' or 'high'
        - 'start_time', 'end_time'
        - 'cilia_times', 'cilia_values'
        - 'actuator_values' (only for 'high')
    """
    times = df[TIME_COL].to_numpy()
    actuator = df[ACTUATOR_COL].to_numpy()
    cilia = df[CILIA_COL].to_numpy()

    # 1) Compute sliding‐window std
    n_t = len(actuator)
    if window_size >= n_t:
        raise ValueError(f"window_size ({window_size}) must be < total frames ({n_t})")
    std_per_window = np.std(
        sliding_window_view(actuator, window_size),
        axis=1
    )

    # 2) Estimate threshold via KDE valley between two peaks
    kde = gaussian_kde(std_per_window)
    grid = np.linspace(std_per_window.min(), std_per_window.max(), 1000)
    dens = kde(grid)
    minima = argrelextrema(dens, np.less)[0]
    maxima = argrelextrema(dens, np.greater)[0]
    if len(minima)==0 or len(maxima)<2:
        raise RuntimeError("Could not detect bimodal distribution in std values.")
    # pick the minimum between the two tallest peaks
    top2 = np.argsort(dens[maxima])[-2:]
    bounds = np.sort(grid[maxima[top2]])
    cand = grid[minima]
    thresh = cand[(cand>bounds[0]) & (cand<bounds[1])][0]

    # 3) Label each window as active/inactive
    is_active = std_per_window > thresh

    # 4) Extract *all* contiguous segments
    all_segs = []
    curr = is_active[0]; start_i = 0
        ...
        if is_active[i] != curr:
            ...
            if e > s:
                seg = {
                    ...
                    'start_time': times[s],
                    'end_time':   times[e],
                    'cilia_times': times[s:e],
                    'cilia_values': cilia[s:e]
                }
                if curr:
                    seg['actuator_values'] = actuator[s:e]
                all_segs.append(seg)
            start_i = i
            curr = is_active[i]
    # last segment
    ...
    if times[-1] > times[s]:
        seg = {
            ...
            'end_time':   times[-1],
            'cilia_times': times[s:],
            'cilia_values': cilia[s:]
        }
        if curr:
            seg['actuator_values'] = actuator[s:]
        all_segs.append(seg)

    # 5) Pick the *largest* high-activity segment
    highs = [sg for sg in all_segs if sg['type']=='high']
    if not highs:
        raise RuntimeError("No high-activity segments found.")
    high_durs = [sg['end_time']-sg['start_time'] for sg in highs]
    high_seg = highs[np.argmax(high_durs)]

    # 6) Pick largest low *before* and *after* it (if they exist)
    lows = [sg for sg in all_segs if sg['type']=='low']
    before = [sg for sg in lows if sg['end_time'] <= high_seg['start_time']]
    after  = [sg for sg in lows if sg['start_time'] >= high_seg['end_time']]

    low_before = None
    if before:
        durs = [sg['end_time']-sg['start_time'] for sg in before]
        low_before = before[np.argmax(durs)]

    low_after = None
    if after:
        durs = [sg['end_time']-sg['start_time'] for sg in after]
        low_after = after[np.argmax(durs)]

    # 7) Interactive plot: STD, threshold, shading + sliders for high start/end
    t_plot = times[:len(std_per_window)]
    fig, ax = plt.subplots(figsize=(10,4))
    ax.plot(t_plot, std_per_window, color='gray', label='Sliding STD')
    ...
    def redraw(sp, ep):
        ...
        if low_before:
            ax.axvspan(low_before['start_time'], sp, color='green', alpha=0.3)
        ax.axvspan(sp, ep, color='orange', alpha=0.3)
        if low_after:
            ax.axvspan(ep, low_after['end_time'], color='green', alpha=0.3)
        ax.set_xlim(t_plot.min(), t_plot.max())
        ax.set_ylim(std_per_window.min(), std_per_window.max()*1.1)
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('STD')
        ax.set_title('Adjust high‐activity window')
        ax.legend()

    # initial
    s0, e0 = high_seg['start_time'], high_seg['end_time']
    redraw(s0, e0)

    # sliders
    axcolor = 'lightgoldenrodyellow'
    ax_s = plt.axes([0.15, 0.02, 0.65, 0.03], facecolor=axcolor)
    ax_e = plt.axes([0.15, 0.06, 0.65, 0.03], facecolor=axcolor)

    slider_s = Slider(ax_s, 'Start High', t_plot.min(), t_plot.max(), valinit=s0)
    slider_e = Slider(ax_e, 'End High',   t_plot.min(), t_plot.max(), valinit=e0)
    ...
    # 8) Return only the three chosen segments
    out = []
    if low_before:
        out.append(low_before)
    out.append(high_seg)
    if low_after:
        out.append(low_after)
    return out
...
def full_analysis(filepath_pattern, show_plots=True):
...
if __name__ == "__main__":
    full_analysis("/home/max/Documents/02_Data/Cilia_project/farzin/csv_to_analyze/*.csv", show_plots=True)